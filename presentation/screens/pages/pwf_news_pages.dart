...
class PwfNewsListWebScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final async = ref.watch(unitNewsProvider(unitSlug));
      ...
      child: PwfSectionContainer(
        ...
        child: async.when(
          data: (items) {
            ...
            return _PwfCardsGrid<NewsArticle>(
              items: items,
              titleBuilder: (a) => a.title,
              subtitleBuilder: (a) => a.excerpt,
              onTap: (a) =>
                  context.go(UnitRoutes.newsDetail(unitSlug, a.id), extra: a),
            );
          },
    ...
    return PwfWebPageScaffold(
      ...
      child: PwfSectionContainer(
        ...
        child: async.when(
          data: (a) {
            if (a == null) {
              return const _PwfEmptyState(message: 'الخبر غير موجود.');
            }
            return _PwfDetailBody(
              title: a.title,
              meta: a.publishedAt ?? a.createdAt,
              content: a.content,
            );
          },
          loading: () => const _PwfLoadingState(),
