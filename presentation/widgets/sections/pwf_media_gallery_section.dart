...
class _PhotosGrid extends ConsumerWidget {
  const _PhotosGrid({super.key, required this.unitSlug});

  final String unitSlug;

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final asyncItems = ref.watch(unitPhotosProvider(unitSlug));
...
class _VideosGrid extends ConsumerWidget {
  const _VideosGrid({super.key, required this.unitSlug});

  final String unitSlug;

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final asyncItems = ref.watch(unitVideosProvider(unitSlug));
    final items = asyncItems.maybeWhen(
      data: (v) => v.where((e) => e.isActive).toList(growable: false),
      orElse: () => const <MediaGalleryItem>[],
    );
    ...
    return _GalleryGrid(
      ...
      onTap: (c) {
        ...
        // Keep exact visuals: open as external/under-construction until a full video player is added.
        context.go(AppRoutes.underConstruction);
      },
    );
  }
...
class _EventsGrid extends ConsumerWidget {
  const _EventsGrid({super.key, required this.unitSlug});
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return _GalleryGrid(
      cards: cards,
      onTap: (_) => context.go(AppRoutes.underConstruction),
    );
  }
}
    ...
    return MouseRegion(
      ...
      child: InkWell(
        ...
        child: AnimatedContainer(
          ...
          transform: _hover ? (Matrix4.identity()..translate(0.0, -3.0)) : null,
