        ...
        children: [
          ...
          Wrap(
            ...
            children: [
              ...
              _metaChip('ربط لاحق مع Mustakshif', Icons.hub_outlined),
            ],
          ),
  ...
  void _openRoute(BuildContext context, String route) {
    if (route.isEmpty || route == '#') {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('سيتم ربط هذه الوجهة في مرحلة لاحقة.')),
      );
      return;
    }
    if (route.startsWith('/')) {
...
class _MiniMapSettings {
  const _MiniMapSettings({
    ...
    this.visualSubtitle = 'سيتم ربطها بـ Mustakshif / GIS لاحقًا',
      ...
      if (key == 'pwf_mini_map_teaser') {
        ...
        return _MiniMapSettings(
          ...
          openMapLabel: (settings['open_map_label'] ?? 'فتح الخريطة')
              .toString(),
          openMapRoute: (settings['open_map_route'] ?? '#').toString(),
          layersLabel: (settings['layers_label'] ?? 'استعراض الطبقات')
              .toString(),
          layersRoute: (settings['layers_route'] ?? '#').toString(),
        );
      }
    }
    return null;
