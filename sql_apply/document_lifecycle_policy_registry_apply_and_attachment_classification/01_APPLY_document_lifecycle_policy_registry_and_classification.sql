...
--
-- Purpose:
-- 1. Create platform_documents.document_types registry.
-- 2. Seed core document type policies.
-- 3. Extend platform_services.service_request_attachments with lifecycle metadata.
-- 4. Classify existing service attachments without deleting data.
-- 5. Refresh /admin/documents public wrappers with lifecycle metadata.
--
-- Boundaries:
-- - No file deletion.
...
-- - No service_role usage in Flutter.
...
comment on schema platform_documents is
  'PalWakf document lifecycle governance schema.';

comment on table platform_documents.document_types is
  'Document type registry for lifecycle, retention, confidentiality, and deletion rules.';
...
select
  'document_lifecycle_apply_result' as section,
