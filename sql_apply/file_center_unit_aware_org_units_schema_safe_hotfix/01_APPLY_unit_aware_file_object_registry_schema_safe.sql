...
--
-- Purpose:
-- Apply unit-aware file object registry with schema-safe org_units projection.
--
-- Hotfix:
-- The wrapper does not directly reference optional org_units columns such as
-- name_ar, slug, type, or active. It uses to_jsonb(ou)->>'field_name'.
--
-- Governance defaults:
-- owner_unit_id = null
...
with object_rows as (
  select
    ...
    'internal'::text as fallback_confidentiality_level
...
select
  ...
  r.mapping_status,
  r.owner_unit_id,
  coalesce(to_jsonb(ou)->>'name_ar', to_jsonb(ou)->>'name') as owner_unit_name_ar,
  to_jsonb(ou)->>'slug' as owner_unit_slug,
  coalesce(to_jsonb(ou)->>'unit_type', to_jsonb(ou)->>'type') as owner_unit_type,
  r.governorate_code,
  r.scope_type,
  r.scope_id,
  r.visibility_scope,
...
select
  'file_center_unit_aware_org_units_schema_safe_apply_result' as section,
