...
with candidates(public_table, target_schema, target_table, sensitivity) as (
  ...
  select
    c.*,
    cls.relrowsecurity as target_rls_enabled,
    cls.relforcerowsecurity as target_force_rls,
    count(pol.oid) as target_policy_count
  from candidates c
  ...
  group by c.public_table, c.target_schema, c.target_table, c.sensitivity, cls.relrowsecurity, cls.relforcerowsecurity
)
select
  'v4_wave1_role_rls_uat_gate_summary' as section,
  count(*) as target_table_count,
  count(*) filter (where target_rls_enabled) as rls_enabled_count,
  count(*) filter (where not target_rls_enabled) as rls_disabled_count,
  count(*) filter (where target_policy_count = 0) as zero_policy_count,
  count(*) filter (where target_policy_count > 0) as with_policy_count,
  count(*) filter (where target_rls_enabled and target_policy_count = 0) as rls_enabled_zero_policy_count,
  count(*) filter (where not target_rls_enabled and sensitivity in ('high','medium-high')) as sensitive_rls_disabled_count,
  'ROLE_RLS_UAT_REQUIRED_BEFORE_PRODUCTION' as decision,
...
select
  'v4_wave1_role_rls_uat_gate_detail' as section,
  public_table,
  target_schema,
  target_table,
  sensitivity,
  ...
  target_policy_count,
  case
    when target_rls_enabled and target_policy_count = 0 then 'RLS_ENABLED_ZERO_POLICY_NEGATIVE_UAT_REQUIRED'
    when not target_rls_enabled and sensitivity in ('high','medium-high') then 'SENSITIVE_TARGET_RLS_DISABLED_OWNER_APPROVAL_REQUIRED'
    when target_policy_count > 0 then 'POLICY_PRESENT_ROLE_MATRIX_UAT_REQUIRED'
