...
with candidates(public_table, target_schema, target_table, sensitivity) as (
  ...
  select
    count(*) as total,
    count(*) filter (where pc.relkind = 'v') as public_views,
    count(*) filter (where tc.relkind = 'r') as target_tables
  from candidates c
  left join pg_namespace pn on pn.nspname = 'public'
  left join pg_class pc on pc.relnamespace = pn.oid and pc.relname = c.public_table
  left join pg_namespace tn on tn.nspname = c.target_schema
  left join pg_class tc on tc.relnamespace = tn.oid and tc.relname = c.target_table
  ...
  where n.nspname not in ('pg_catalog','information_schema')
    and p.prokind in ('f','p') -- exclude aggregate/window functions such as PostGIS ST_Union
  ...
  select count(*) as text_dependency_rows
  from candidates c
  join function_defs fd on fd.function_body ilike ('%' || c.public_table || '%')
), rls as (
  select
    count(*) filter (where cls.relrowsecurity and coalesce(policy_count,0)=0) as rls_enabled_zero_policy_count,
    count(*) filter (where not cls.relrowsecurity and c.sensitivity in ('high','medium-high')) as sensitive_rls_disabled_count
  from candidates c
  join pg_namespace n on n.nspname = c.target_schema
  join pg_class cls on cls.relnamespace = n.oid and cls.relname = c.target_table
  left join lateral (select count(*) as policy_count from pg_policy pol where pol.polrelid = cls.oid) pols on true
)
select
  'v4_wave1_production_gate_decision' as section,
  shape.total as wave1_count,
  shape.public_views as public_compatibility_views,
  shape.target_tables as owner_schema_tables,
  deps.text_dependency_rows,
  rls.rls_enabled_zero_policy_count,
  rls.sensitive_rls_disabled_count,
  false as compatibility_views_removal_authorized,
  false as wave3_move_authorized,
  false as production_approved,
  'PRODUCTION_BLOCKED_PENDING_RUNTIME_DEPENDENCY_REMEDIATION_GRANT_REVIEW_ROLE_RLS_UAT_BROWSER_UAT' as decision,
  true as read_only
from shape, deps, rls;
