...
select
  'v4_move_target_schema_collision_detail' as section,
  m.public_table,
  m.target_schema,
  m.target_table,
  ...
  case
    when to_regnamespace(m.target_schema) is null then 'TARGET_SCHEMA_MISSING_CREATE_SCHEMA_REQUIRED_IN_AUTHORIZED_APPLY'
    when to_regclass(format('%I.%I', m.target_schema, m.target_table)) is not null then 'BLOCKED_TARGET_TABLE_NAME_COLLISION'
    else 'MOVE_NAME_AVAILABLE'
  end as move_readiness,
...
select
  'v4_move_target_schema_collision_summary' as section,
  ...
  not bool_or(target_table_name_collision) as no_target_table_collisions,
  false as apply_authorized_by_this_script,
  false as destructive_sql_authorized,
  false as production_approved,
  true as read_only
