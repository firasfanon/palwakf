...
with v4_move_map(public_table, target_schema, target_table, sensitivity, functional_reason) as (
  ...
  from v4_move_map m
  join pg_class c on c.oid = to_regclass(format('public.%I', m.public_table))
), policy_inv as (
  select
    t.public_table, t.target_schema, t.target_table,
    pol.policyname, pol.cmd, pol.roles, pol.qual, pol.with_check
  from table_oids t
  left join pg_policies pol on pol.schemaname = 'public' and pol.tablename = t.public_table
), rls as (
  select t.public_table, t.target_schema, t.target_table, c.relrowsecurity, c.relforcerowsecurity
  from table_oids t join pg_class c on c.oid = t.table_oid
)
select
  'v4_move_rls_policy_inventory_detail' as section,
  r.public_table, r.target_schema, r.target_table,
  r.relrowsecurity as rls_enabled,
  r.relforcerowsecurity as rls_forced,
  p.policyname, p.cmd, p.roles,
  'VERIFY_RLS_POLICIES_AND_GRANTS_AFTER_SET_SCHEMA' as required_action,
  false as apply_authorized_by_this_script,
  false as production_approved,
  true as read_only
from rls r
left join policy_inv p on p.public_table = r.public_table
order by r.target_schema, r.public_table, p.policyname;
...
with v4_move_map(public_table, target_schema, target_table, sensitivity, functional_reason) as (
  ...
  select t.*, c.relrowsecurity, c.relforcerowsecurity
  from table_oids t join pg_class c on c.oid = t.table_oid
), pol as (
...
select
  'v4_move_rls_policy_inventory_summary' as section,
  count(*) as mapped_public_table_count,
  count(*) filter (where relrowsecurity) as rls_enabled_table_count,
  count(*) filter (where coalesce(pol.policy_count,0) > 0) as tables_with_policies,
  sum(coalesce(pol.policy_count,0)) as total_policy_count,
  false as apply_authorized_by_this_script,
  false as production_approved,
  true as read_only
from rls
left join pol on pol.schemaname = 'public' and pol.tablename = rls.public_table;
