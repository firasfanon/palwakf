...
with v4_move_map(public_table, target_schema, target_table, sensitivity, functional_reason) as (
  ...
  select
    m.public_table, m.target_schema, m.target_table,
    f.function_schema, f.function_name, f.arguments,
    case
      when position(format('public.%I', m.public_table) in f.function_definition) > 0 then 'qualified_identifier'
      when position('public.' || m.public_table in f.function_definition) > 0 then 'qualified_text'
      when position(m.public_table in f.function_definition) > 0 then 'unqualified_text_possible_false_positive'
      else null
    end as match_type
  from v4_move_map m
  join fn f on position(m.public_table in f.function_definition) > 0
)
select
  'v4_move_function_rpc_text_dependency_detail' as section,
  public_table, target_schema, target_table,
  function_schema, function_name, arguments, match_type,
  'REVIEW_FUNCTION_TEXT_AND_REWRITE_TO_OWNER_SCHEMA_OR_PUBLIC_COMPAT_VIEW' as required_action,
  false as apply_authorized_by_this_script,
  false as production_approved,
  true as read_only
from hits
where match_type is not null
order by public_table, function_schema, function_name;
...
select
  'v4_move_function_rpc_text_dependency_summary' as section,
  count(*) as function_text_dependency_rows,
  count(distinct public_table) as public_tables_with_function_text_dependencies,
  count(distinct function_schema || '.' || function_name) as dependent_functions,
  false as apply_authorized_by_this_script,
  false as production_approved,
  true as read_only
from hits;
