...
with v4_move_map(public_table, target_schema, target_table, sensitivity, functional_reason) as (
  ...
  select t.*, a.attnum, a.attname,
         pg_get_expr(ad.adbin, ad.adrelid) as default_expr
  from table_oids t
  join pg_attribute a on a.attrelid = t.table_oid and a.attnum > 0 and not a.attisdropped
  left join pg_attrdef ad on ad.adrelid = t.table_oid and ad.adnum = a.attnum
), seq_deps as (
  select
    a.public_table, a.target_schema, a.target_table, a.attname,
    a.default_expr,
    seq_ns.nspname as owned_sequence_schema,
    seq.relname as owned_sequence_name
  from attrs a
  left join pg_depend d
    on d.refobjid = a.table_oid
  ...
  where a.default_expr is not null or seq.oid is not null
)
select
  'v4_move_sequence_default_dependency_detail' as section,
  public_table, target_schema, target_table, attname,
  default_expr,
  owned_sequence_schema,
  owned_sequence_name,
  'VERIFY_SEQUENCE_MOVES_OR_REBIND_DEFAULT_IN_APPLY_PLAN' as required_action,
  false as apply_authorized_by_this_script,
  false as production_approved,
  true as read_only
from seq_deps
order by target_schema, public_table, attname;
...
select
  'v4_move_sequence_default_dependency_summary' as section,
  count(*) filter (where default_expr is not null) as columns_with_defaults,
  count(distinct public_table) filter (where default_expr is not null) as public_tables_with_defaults,
  'READ_ONLY_REVIEW_REQUIRED' as decision,
  false as apply_authorized_by_this_script,
  false as production_approved,
  true as read_only
from attrs;
