...
select distinct
  'v4_move_structural_view_dependency_detail' as section,
  public_table, target_schema, target_table,
  dependent_schema, dependent_view, dependent_kind,
  'VERIFY_VIEW_REMAINS_VALID_AFTER_SET_SCHEMA_AND_PLAN_COMPATIBILITY_SURFACE' as required_action,
  false as archive_authorized,
  false as apply_authorized_by_this_script,
  false as production_approved,
  true as read_only
from view_deps
order by public_table, dependent_schema, dependent_view;
...
select
  'v4_move_structural_view_dependency_summary' as section,
  count(*) as structural_view_dependency_rows,
  count(distinct public_table) as public_tables_with_view_dependencies,
