...
-- 02_SERVICE_CENTER_ONE_SHOT_CONTROLLED_APPLY_GUARDED.sql
-- Purpose: one-shot controlled owner apply only if 01 proves required Service Center gaps.
-- Guarded: do not run without backup and explicit token replacement.

DO $$
BEGIN
  IF 'REPLACE_WITH_PWF_PHASE_C_SERVICE_CENTER_APPROVAL_TOKEN' = 'REPLACE_WITH_PWF_PHASE_C_SERVICE_CENTER_APPROVAL_TOKEN' THEN
    RAISE EXCEPTION 'BLOCKED: Phase C Service Center controlled apply requires explicit operator token replacement after reviewing 01 census and confirming a fresh database backup.';
  END IF;
  IF 'BACKUP_NOT_CONFIRMED' <> 'BACKUP_CONFIRMED' THEN
    RAISE EXCEPTION 'BLOCKED: Phase C Service Center controlled apply requires backup marker BACKUP_CONFIRMED.';
  END IF;
END $$;

-- Safety boundary: this script installs/repairs platform_services owner objects and public RPC wrappers only.
-- It does not drop/delete/archive/exact-replace legacy public tables.
-- It does not touch auth.users, service_role secrets, waqf, awqaf_system, or GIS.
