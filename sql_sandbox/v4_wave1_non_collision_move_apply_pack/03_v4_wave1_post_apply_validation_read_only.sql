...
-- Validate state after Wave1 guarded apply. Read-only.
with move_map(public_table, target_schema, target_table, sensitivity) as (
  ...
  select
    m.*,
    to_regclass(format('public.%I', m.public_table)) is not null as public_compat_object_present,
    (select c.relkind from pg_class c join pg_namespace n on n.oid=c.relnamespace where n.nspname='public' and c.relname=m.public_table limit 1) as public_object_kind,
    to_regclass(format('%I.%I', m.target_schema, m.target_table)) is not null as target_table_present,
    (select c.relkind from pg_class c join pg_namespace n on n.oid=c.relnamespace where n.nspname=m.target_schema and c.relname=m.target_table limit 1) as target_object_kind
  from move_map m
)
select
  'v4_wave1_post_apply_validation_summary' as section,
  count(*) as wave1_candidate_count,
  count(*) filter (where target_table_present and target_object_kind in ('r','p')) as moved_target_tables_present,
  count(*) filter (where public_compat_object_present and public_object_kind='v') as public_compat_views_present,
  count(*) filter (where public_object_kind in ('r','p')) as public_base_tables_remaining_for_wave1,
  count(*) filter (where not target_table_present) as missing_target_tables_after_apply,
  (count(*) filter (where target_table_present and target_object_kind in ('r','p')) = 88
    and count(*) filter (where public_compat_object_present and public_object_kind='v') = 88
    and count(*) filter (where public_object_kind in ('r','p')) = 0
  ) as wave1_post_apply_state_valid,
...
select
  'v4_wave1_post_apply_validation_detail' as section,
  m.public_table,
  m.target_schema,
  m.target_table,
  (select c.relkind from pg_class c join pg_namespace n on n.oid=c.relnamespace where n.nspname='public' and c.relname=m.public_table limit 1) as public_object_kind,
