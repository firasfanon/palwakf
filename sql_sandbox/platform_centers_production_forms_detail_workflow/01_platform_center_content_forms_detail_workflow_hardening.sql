-- Mega Batch L — Platform Centers Production Forms + Detail Pages + Workflow Hardening
-- Scope: production-form persistence, public detail RPC, and DB-side workflow state-machine hardening.
-- This script does not touch waqf.*, awqaf_system tables, or mustakshif.

begin;

-- Keep the public view published-only, but include fields needed by detail pages.
...
-- Public/admin detail RPC. Public path remains published-only; admin path requires media_center_can_read_v1().
...
-- Replace the upsert wrapper with the production form contract. It remains backward-compatible by defaulting extra params.
...
begin
  ...
  insert into platform_content.center_content_workflow_events (
    ...
    case when v_is_update then 'unchanged' else null end,
    'draft',
...
-- Harden workflow transitions at DB/RPC level; UI buttons are not the source of truth.
...
begin
  ...
  insert into platform_content.center_content_workflow_events (
    content_item_id, family_key, action_key, from_status, to_status, decision_label_ar, actor_id, unit_slug, source_route, notes, metadata
  ) values (
    v_id,
    v_family_key,
    v_action,
  ...
  return jsonb_build_object('success', true, 'id', v_id::text, 'from_status', v_current_status, 'to_status', v_new_status, 'message_ar', 'تم تحديث حالة سير العمل.');
end;
$$;
