...
checks as (
  select
    m.*,
    exists(select 1 from pg_namespace n where n.nspname = m.target_schema) as target_schema_exists,
    to_regclass(format('%I.%I', m.target_schema, m.target_table)) is not null as target_table_exists,
    to_regclass(format('public.%I', m.public_table)) is not null as public_table_exists
  from v4_sensitive_map m
)
select
  'v4_sensitive_target_schema_presence_detail' as section,
  public_table,
  target_schema,
  target_table,
  public_table_exists,
...
select
  'v4_sensitive_target_schema_presence_summary' as section,
  count(*) as mapped_public_table_count,
  count(*) filter (where public_table_exists) as public_tables_present,
  count(*) filter (where target_schema_exists) as target_schema_present_count,
  count(*) filter (where target_table_exists) as target_table_present_count,
  bool_and(public_table_exists) as all_public_tables_present,
  bool_and(target_schema_exists) as all_target_schemas_present,
  bool_and(target_table_exists) as all_target_tables_present,
