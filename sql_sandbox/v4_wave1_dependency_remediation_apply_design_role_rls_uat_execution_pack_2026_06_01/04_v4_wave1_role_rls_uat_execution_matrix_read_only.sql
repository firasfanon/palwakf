...
matrix as (
  select
    ...
    coalesce(r.force_rls, false) as target_force_rls,
    coalesce(p.policy_count, 0) as target_policy_count,
    case
      when w.sensitivity in ('high','medium-high') and coalesce(r.rls_enabled, false) = false
        then 'SENSITIVE_TARGET_RLS_DISABLED_OWNER_APPROVAL_REQUIRED'
      when coalesce(r.rls_enabled, false) = true and coalesce(p.policy_count, 0) = 0
        then 'RLS_ENABLED_ZERO_POLICY_NEGATIVE_UAT_REQUIRED'
      when coalesce(r.rls_enabled, false) = true and coalesce(p.policy_count, 0) > 0
        then 'POLICY_PRESENT_ROLE_MATRIX_UAT_REQUIRED'
      else 'ROLE_UAT_REVIEW_REQUIRED'
    end as uat_gate,
    case
      when w.target_schema in ('platform_access','awqaf_system','waqf','core','topology') or w.sensitivity in ('high','medium-high')
        then 'anon_negative + authenticated_negative + authorized_role_positive + wrong_unit_negative + audit_trace_review'
      else 'anon_negative + authenticated_expected_read + authorized_admin_positive + browser_smoke'
    end as required_uat_scenarios,
...
select
  'v4_wave1_role_rls_uat_execution_matrix_detail' as section,
  *
from matrix
order by
  case uat_gate
    when 'SENSITIVE_TARGET_RLS_DISABLED_OWNER_APPROVAL_REQUIRED' then 1
    when 'RLS_ENABLED_ZERO_POLICY_NEGATIVE_UAT_REQUIRED' then 2
    when 'POLICY_PRESENT_ROLE_MATRIX_UAT_REQUIRED' then 3
    else 4
  end,
  sensitivity desc,
  target_schema,
