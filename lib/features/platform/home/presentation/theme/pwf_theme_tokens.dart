...
class PwfThemeTokens {
  ...
  static PwfThemeTokensData forVisualContext(
    PwfVisualContext context, {
    PwfVisualFamily? family,
  }) {
    final preset = PwfVisualIdentityRegistry.defaults.firstWhere(
      (item) =>
          item.context == context && (family == null || item.family == family),
      orElse: () => PwfVisualIdentityRegistry.defaultForContext(context),
    );
    return preset.toHomeTokens();
  }
