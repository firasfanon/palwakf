...
class RbacAdminRepository {
  ...
  static const String _platformSystemsLegacyWriteTable = 'platform_systems';
  ...
  Future<void> ensureSystemRegistered(String systemKey) async {
    final normalized = systemKey.trim();
    if (normalized.isEmpty) return;

    final isGoverned = SystemKey.values.any((e) => e.name == normalized);
    if (!isGoverned) {
      throw ArgumentError('system_key غير معتمد في المنصة: $normalized');
    }

    await _client.from(_platformSystemsLegacyWriteTable).upsert({
      'key': normalized,
    });
  }
  ...
  Future<void> upsertUserRole({
    ...
    await ensureSystemRegistered(systemKey);
