...
class CaseRepository {
  ...
  Future<List<Case>> getAllCases({int? limit, int? offset}) async {
    try {
      dev.log('Fetching cases', name: 'CaseRepository');

      var query = _supabaseService.client
          .from(PwfDatabaseOwnerSurfaces.cases)
          .select()
      ...
      dev.log(
        'Successfully fetched ${(response as List).length} cases',
        name: 'CaseRepository',
      );
  ...
  Future<Case?> getCaseByCaseNumber(String caseNumber) async {
    try {
      ...
      if (response == null) {
        dev.log(
          'Case not found with number: $caseNumber',
          name: 'CaseRepository',
        );
        return null;
      }
  ...
  Future<List<Case>> getCasesByStatus(CaseStatus status, {int? limit}) async {
    try {
      dev.log(
        'Fetching cases with status: ${status.name}',
        name: 'CaseRepository',
      );
      ...
      dev.log(
        'Error fetching cases by status',
        name: 'CaseRepository',
        error: e,
        stackTrace: stackTrace,
      );
  ...
  Future<List<Case>> getCasesByPriority(
    CasePriority priority, {
    int? limit,
  }) async {
    try {
      dev.log(
        'Fetching cases with priority: ${priority.name}',
        name: 'CaseRepository',
      );
      ...
      dev.log(
        'Error fetching cases by priority',
        name: 'CaseRepository',
        error: e,
        stackTrace: stackTrace,
      );
  ...
  Future<List<Case>> getCasesByGovernorate(
    String governorate, {
    int? limit,
  }) async {
    try {
      dev.log(
        'Fetching cases for governorate: $governorate',
        name: 'CaseRepository',
      );
      ...
      dev.log(
        'Error fetching cases by governorate',
        name: 'CaseRepository',
        error: e,
        stackTrace: stackTrace,
      );
  ...
  Future<List<Case>> searchCases(String query) async {
    try {
      ...
      final response = await _supabaseService.client
          ...
          .select()
          .or(
            'title.ilike.%$query%,description.ilike.%$query%,case_number.ilike.%$query%',
          )
          .order('filing_date', ascending: false);
  ...
  Future<Case> createCase(Case caseData) async {
    try {
      dev.log(
        'Creating new case: ${caseData.caseNumber}',
        name: 'CaseRepository',
      );
      ...
      dev.log(
        'Case created successfully with ID: ${response['id']}',
        name: 'CaseRepository',
      );
  ...
  Future<Case> updateCaseStatus(int id, CaseStatus status) async {
    try {
      dev.log(
        'Updating case status to: ${status.name}',
        name: 'CaseRepository',
      );
