...
import '../../../data/repositories/homepage_repository.dart'
    show BreakingNewsItem, BreakingNewsSectionSettings;
...
class _BreakingNewsSliderState extends ConsumerState<BreakingNewsSlider>
  ...
  final double _scrollVelocity =
      50.0; // Pixels per second (increased for faster scroll)
  ...
  Widget build(BuildContext context) {
    final breakingNewsAsync = ref.watch(
      activeBreakingNewsForUnitProvider(widget.unitSlug),
    );
        ...
        child: Row(
          children: [
            ...
            _BreakingNewsLabel(
              ...
              iconName: settings.defaultIcon.isEmpty
                  ? 'bullhorn'
...
class _BreakingNewsLabel extends StatelessWidget {
  ...
  const _BreakingNewsLabel({required this.showIcon, required this.iconName});
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Row(
        children: [
          if (showIcon) ...[
            Icon(_getIcon(iconName), color: Colors.white, size: 24),
  ...
  Widget build(BuildContext context) {
    return ListView.separated(
      ...
      itemBuilder: (context, index) {
        ...
        return _BreakingNewsItemWidget(item: item, settings: settings);
      },
    );
  }
}
...
class _BreakingNewsItemWidget extends StatelessWidget {
  ...
  const _BreakingNewsItemWidget({required this.item, required this.settings});
  ...
  Widget build(BuildContext context) {
    ...
    final content = Row(
      children: [
        if (item.icon.isNotEmpty) ...[
          Icon(_getIcon(item.icon), color: textColor, size: 18),
          const SizedBox(width: 8),
        ],
        Text(
          item.text,
    ...
    if (settings.allowClick && item.link.trim().isNotEmpty) {
      return InkWell(
        onTap: () {
