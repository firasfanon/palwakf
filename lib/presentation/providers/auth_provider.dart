...
class AuthStateNotifier extends StateNotifier<AuthState> {
  ...
  Future<void> resetPassword(String email, {String? redirectTo}) async {
    ...
    try {
      await _authRepository.resetPassword(email, redirectTo: redirectTo);
  ...
  /// Exchange Supabase recovery code into a temporary recovery session.
  Future<void> exchangeRecoveryCode(String code) async {
    state = state.copyWith(isLoading: true, clearError: true);

    try {
      await _authRepository.exchangeRecoveryCode(code);
      state = state.copyWith(isLoading: false, clearError: true);
    } catch (e) {
      state = state.copyWith(
        isLoading: false,
  ...
  /// Clear local auth state without calling Supabase.
...
/// Authenticated if Supabase has a current user OR our local state says so.
/// This avoids false negatives on initial load before initialize() finishes.
final isAuthenticatedProvider = Provider<bool>((ref) {
  final supaUser = supabase.Supabase.instance.client.auth.currentUser;
  if (supaUser != null) return true;
