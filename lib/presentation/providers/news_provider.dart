...
final newsByCategoryProvider =
    FutureProvider.family<List<NewsArticle>, NewsCategory>((
      ref,
      category,
    ) async {
      final newsService = ref.read(newsServiceProvider);
...
final newsStatisticsProvider = FutureProvider<Map<String, dynamic>>((
  ref,
) async {
  final newsService = ref.read(newsServiceProvider);
...
final newsFilterProvider =
    StateNotifierProvider<NewsFilterNotifier, NewsFilter>(
  ...
  if (filter.searchQuery.isNotEmpty) {
    articles = articles.where((article) {
      return article.title.toLowerCase().contains(
          ...
          article.excerpt.toLowerCase().contains(
            filter.searchQuery.toLowerCase(),
          );
    }).toList();
  }
...
final paginatedNewsProvider =
    StateNotifierProvider<PaginatedNewsNotifier, PaginatedNewsState>(
...
final recentlyViewedNewsProvider =
    StateNotifierProvider<RecentlyViewedNotifier, List<NewsArticle>>(
...
final newsBookmarksProvider =
    StateNotifierProvider<NewsBookmarksNotifier, List<NewsArticle>>(
