...
final heroSlidesProvider =
    AsyncNotifierProvider<HeroSlidesNotifier, List<HeroSlide>>(() {
...
class HeroSliderManagementScreen extends ConsumerStatefulWidget {
  const HeroSliderManagementScreen({super.key, this.embedded = false});
...
class _HeroSliderManagementScreenState
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.embedded) {
      return Material(color: AppColors.surfaceVariant, child: body);
    }

    return Scaffold(
      backgroundColor: AppColors.surfaceVariant,
      body: Row(children: [Expanded(child: body)]),
    );
  }
            ...
            children: [
              ...
              Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 8),
                  Text(
                    slide.ctaText.isEmpty ? 'اقرأ المزيد' : slide.ctaText,
                    style: const TextStyle(
                      fontSize: 12,
                      color: AppConstants.islamicGreen,
  ...
  Widget _buildEmptyState() {
    return Center(
      child: Column(
        ...
        children: [
          Icon(Icons.slideshow, size: 100, color: Colors.grey[300]),
          const SizedBox(height: 24),
          Text(
            'لا توجد شرائح بعد',
            style: TextStyle(
  ...
  void _handleToggleActive(HeroSlide slide) async {
    ...
    await ref
        .read(heroSlidesProvider.notifier)
        .updateSlide(slide.copyWith(isActive: !slide.isActive));
  ...
  void _handleBulkDelete() {
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        title: const Text('تأكيد الحذف'),
        content: Text('هل أنت متأكد من حذف ${_selectedSlides.length} شريحة؟'),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(context),
            child: const Text('إلغاء'),
          ...
          ElevatedButton(
            onPressed: () async {
              ...
              await ref
                  .read(heroSlidesProvider.notifier)
                  .deleteSlides(_selectedSlides.toList());
...
class _SlideFormDialog extends StatefulWidget {
  ...
  const _SlideFormDialog({this.slide, required this.onSave});
...
class _SlideFormDialogState extends State<_SlideFormDialog> {
  ...
  void initState() {
    ...
    _ctaTextController = TextEditingController(
      text: slide?.ctaText ?? 'اقرأ المزيد',
    );
