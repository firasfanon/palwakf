...
class BreakingNewsManagementScreen extends ConsumerStatefulWidget {
  const BreakingNewsManagementScreen({super.key, this.embedded = false});
...
class _BreakingNewsManagementScreenState
  ...
  Widget _buildStatsBar(List<BreakingNewsItem> allItems) {
    ...
    final expiredCount = allItems
        .where(
          (s) => s.expiresAt != null && s.expiresAt!.isBefore(DateTime.now()),
        )
        .length;
  ...
  Widget _buildEmptyState() {
    return Center(
      child: Column(
        ...
        children: [
          Icon(Icons.campaign, size: 100, color: Colors.grey[300]),
          const SizedBox(height: 24),
          Text(
  ...
  void _handleToggleActive(BreakingNewsItem item) async {
    ...
    await ref
        ...
        .updateItem(item.copyWith(isActive: !item.isActive));

    if (mounted) {
      messenger.showSnackBar(
        SnackBar(
          content: Text(item.isActive ? 'تم إخفاء الخبر' : 'تم إظهار الخبر'),
  ...
  void _handleBulkDelete() {
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        title: const Text('تأكيد الحذف'),
        content: Text('هل أنت متأكد من حذف ${_selectedItems.length} خبر؟'),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(context),
            child: const Text('إلغاء'),
          ...
          ElevatedButton(
            onPressed: () async {
              ...
              await ref
                  .read(breakingNewsProvider.notifier)
                  .deleteItems(_selectedItems.toList());
