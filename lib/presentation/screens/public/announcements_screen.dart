...
class AnnouncementsScreen extends ConsumerStatefulWidget {
  ...
  const AnnouncementsScreen({super.key, required this.unitSlug});
...
class _AnnouncementsScreenState extends ConsumerState<AnnouncementsScreen> {
  ...
  Widget build(BuildContext context) {
    final asyncAnnouncements = ref.watch(
      announcementsForUnitProvider(widget.unitSlug),
    );
        ...
        children: [
          ...
          Expanded(
            child: asyncAnnouncements.when(
              loading: () => const Center(child: CircularProgressIndicator()),
              error: (e, _) =>
                  const Center(child: Text('تعذّر تحميل الإعلانات')),
              data: (rows) {
                final items = _applyFilters(rows);
          ...
          SizedBox(
            ...
            child: TextField(
              ...
              decoration: InputDecoration(
                ...
                prefixIcon: Icon(
                  Icons.search,
                  color: onDark ? Colors.white : Colors.grey[700],
                ),
                hintStyle: TextStyle(
          ...
          Wrap(
            ...
            children: [
              ...
              AppFilterChip(
                ...
                onSelected: () =>
                    setState(() => _selectedPriority = Priority.critical),
                onDarkBackground: onDark,
              ),
              AppFilterChip(
                label: 'عاجل',
                ...
                onSelected: () =>
                    setState(() => _selectedPriority = Priority.urgent),
                onDarkBackground: onDark,
              ),
              AppFilterChip(
                label: 'عالي',
              ...
              AppFilterChip(
                label: 'عادي',
                isSelected: _selectedPriority == Priority.normal,
                onSelected: () =>
                    setState(() => _selectedPriority = Priority.normal),
                onDarkBackground: onDark,
              ),
              AppFilterChip(
                label: 'منخفض',
      ...
      child: Wrap(
        ...
        children: announcements
            .map(
              (a) => SizedBox(
                width: 520,
                child: _announcementCard(a, isWeb: true),
              ),
            )
            .toList(),
      ),
  ...
  Color _getPriorityColor(Priority priority) {
    switch (priority) {
      ...
      case Priority.normal:
        return const Color(0xFF2563EB); // Balanced blue
      case Priority.low:
        return Colors.grey;
    }
  }
