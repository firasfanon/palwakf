...
class AppRouter {
  ...
  static const String adminActivitiesManagement =
  ...
  static const String adminFridaySermons = '/admin/friday-sermons';

  static Route<dynamic> generateRoute(RouteSettings settings) {
    // Many public screens are unit-aware (e.g. unitSlug='home').
    // Support passing unitSlug via Navigator arguments.
    String _unitSlugFromArgs({String fallback = 'home'}) {
      final args = settings.arguments;
      if (args is String && args.trim().isNotEmpty) return args.trim();
      if (args is Map) {
        final v = args['unitSlug'];
    ...
    switch (settings.name) {
      ...
      case news:
        return MaterialPageRoute(
          builder: (_) => NewsScreen(unitSlug: _unitSlugFromArgs()),
          settings: settings,
        );
      ...
      case announcements:
        return MaterialPageRoute(
          builder: (_) => AnnouncementsScreen(unitSlug: _unitSlugFromArgs()),
          settings: settings,
        );
      ...
      case adminDashboard:
        return MaterialPageRoute(
          builder: (_) => const AuthGuardedRoute(child: DashboardScreen()),
          settings: settings,
        );
      ...
      case adminWaqfLands:
        return MaterialPageRoute(
          builder: (_) => const AuthGuardedRoute(child: WaqfLandsScreen()),
          settings: settings,
        );
      ...
      case adminCases:
        return MaterialPageRoute(
          builder: (_) => const AuthGuardedRoute(child: CasesScreen()),
          settings: settings,
        );
      ...
      case adminDocuments:
        return MaterialPageRoute(
          builder: (_) => const AuthGuardedRoute(child: DocumentsScreen()),
          settings: settings,
        );

      case adminProfile:
        return MaterialPageRoute(
          builder: (_) => const AuthGuardedRoute(child: ProfileScreen()),
          settings: settings,
        );

      case adminHomeManagement:
        return MaterialPageRoute(
          builder: (_) => const AuthGuardedRoute(child: HomeManagementScreen()),
          settings: settings,
        );
      ...
      case adminHeroSlider:
        return MaterialPageRoute(
          builder: (_) =>
              const AuthGuardedRoute(child: HeroSliderManagementScreen()),
          settings: settings,
        );
      ...
      case adminBreakingNews:
        return MaterialPageRoute(
          builder: (_) =>
              const AuthGuardedRoute(child: BreakingNewsManagementScreen()),
          settings: settings,
        );

      // Activities Management (Auth Required) ✅

      case adminActivitiesManagement:
        return MaterialPageRoute(
          builder: (_) =>
              const AuthGuardedRoute(child: ActivitiesManagementScreen()),
          settings: settings,
        );
  ...
  /// Alias for navigateToAndReplace
  static Future<T?> pushReplacement<T extends Object?, TO extends Object?>(
    ...
    return navigateToAndReplace<T, TO>(
      context,
      routeName,
      result: result,
      arguments: arguments,
  ...
  /// Alias for navigateToAndClearStack
  static Future<T?> pushAndClearStack<T extends Object?>(
    BuildContext context,
    String routeName, {
    Object? arguments,
  }) {
    return navigateToAndClearStack<T>(context, routeName, arguments: arguments);
  }
...
class AuthGuardedRoute extends ConsumerStatefulWidget {
  ...
  const AuthGuardedRoute({super.key, required this.child});
...
class _AuthGuardedRouteState extends ConsumerState<AuthGuardedRoute> {
  ...
  // FIXED: Made async and initialize auth provider first
  void _checkAuthentication() async {
    // Initialize auth provider first (only runs once due to _initialized flag)
    ...
    if (!isAuthenticated) {
      ...
      if (mounted) {
        Navigator.of(
          context,
        ).pushNamedAndRemoveUntil(AppRouter.adminLogin, (route) => false);
      }
    }
  }
